...
pub unsafe extern "C" fn import_user(
    ...
    CStr::from_ptr(c_password)
        .to_str()
        .map_or(Err::BadArgument, |password| {
            transport.as_mut().map_or(Err::NullArgument, |tsp| {
                c_user.as_mut().map_or(Err::NullArgument, |u| {
                    let bytes_vec: Vec<_> = buffer.into();
                    run_async(User::restore(&bytes_vec, password, tsp.clone())).map_or(Err::OperationFailed, |user| {
                        *u = safe_into_mut_ptr(user);
                        Err::Ok
                    })
                })
        ...
        .map_or(Err::BadArgument, |password| {
            c_user.as_mut().map_or(Err::NullArgument, |user| {
                buf.as_mut().map_or(Err::NullArgument, |buf| {
                    run_async(user.backup(password)).map_or(Err::OperationFailed, |bytes| {
                        *buf = bytes.into();
                        Err::Ok
                    })
                })
...
pub unsafe extern "C" fn create_stream(addr: *mut *const Address, user: *mut User, base_branch: *const c_char) -> Err {
    CStr::from_ptr(base_branch)
        .to_str()
        .map_or(Err::BadArgument, |base_branch| {
...
#[no_mangle]
pub unsafe extern "C" fn subscribe(sub_link: *mut *const Address, user: *mut User) -> Err {
    sub_link.as_mut().map_or(Err::NullArgument, |s_link| {
        user.as_mut().map_or(Err::NullArgument, |user| {
            run_async(user.subscribe()).map_or(Err::OperationFailed, |resp| -> Err {
...
/// `OperationFailed` error will be returned.
#[no_mangle]
pub unsafe extern "C" fn unsubscribe(unsub_link: *mut *const Address, user: *mut User) -> Err {
    unsub_link.as_mut().map_or(Err::NullArgument, |u_link| {
        user.as_mut().map_or(Err::NullArgument, |user| {
            run_async(user.unsubscribe()).map_or(Err::OperationFailed, |resp| -> Err {
...
pub unsafe extern "C" fn new_branch(
    ...
    r.as_mut().map_or(Err::NullArgument, |r| {
        user.as_mut().map_or(Err::NullArgument, |user| {
            CStr::from_ptr(old_topic)
                .to_str()
                .map_or(Err::BadArgument, |old_topic| {
                    CStr::from_ptr(new_topic)
                        .to_str()
                        .map_or(Err::BadArgument, |new_topic| {
                            run_async(user.new_branch(old_topic, new_topic)).map_or(Err::OperationFailed, |resp| {
    ...
    r.as_mut().map_or(Err::NullArgument, |r| {
        user.as_mut().map_or(Err::NullArgument, |user| {
            CStr::from_ptr(topic).to_str().map_or(Err::BadArgument, |topic| {
                ...
                let mut message = user.message().with_topic(topic).with_payload(payload);
                ...
                run_async(message.send()).map_or(Err::OperationFailed, |response| {
                    *r = safe_into_ptr(response.address());
                    Err::Ok
                })
            })
...
/// Receive and process a message via a given address. This is used to process a message with a
/// known location, rather than through discovery. This is relevant for retrieving specific message
/// details after mapping in an external application/store. Used to receive the `Announcement`
/// messages that designate the root of a stream. If the message cannot be found, an
/// `OperationFailed` message is returned.
#[no_mangle]
pub unsafe extern "C" fn receive_message(
    ...
    message.as_mut().map_or(Err::NullArgument, |m| {
        address.as_ref().map_or(Err::NullArgument, |addr| {
            user.as_mut()
                .map_or(Err::NullArgument, |user| match run_async(user.receive_message(*addr)) {
                    Ok(msg) => {
...
/// struct and returned.
#[no_mangle]
pub unsafe extern "C" fn fetch_next_message(message: *mut *const Message, user: *mut User) -> Err {
    message.as_mut().map_or(Err::NullArgument, |m| {
        user.as_mut().map_or(Err::NullArgument, |user| {
            run_async(user.messages().next()).map_or(Err::OperationFailed, |message| match message {
                Ok(msg) => {
                    let msg_wrap: Message = msg.into();
                    *m = safe_into_ptr(msg_wrap);
                    Err::Ok
...
/// Check message for role in branch. Returns Bad Argument if the message is not a keyload. Fails
/// the operation if the message cannot be retrieved via the address provided in it, or if the user
/// is not subscribed to the keyload.
#[no_mangle]
    ...
    user_role.as_mut().map_or(Err::NullArgument, |user_role| {
        message.as_ref().map_or(Err::NullArgument, |message| {
            message
                .address
                .as_ref()
                .map_or(Err::NullArgument, |address| match message.message_type {
                    MessageType::Keyload => user.as_mut().map_or(Err::NullArgument, |user| {
                        match run_async(user.receive_message(*address)) {
                            Ok(keyload) => match keyload.content {
                                MessageContent::Keyload(k) => k
                                    .subscribers
                                    .iter()
